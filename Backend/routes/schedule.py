...
def _get_authenticated_tutor():
    """Valida el token y que el rol sea tutor. Retorna el id_usuario o error."""
    ...
    try:
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                # Verificar si el tutor ya tiene horario
                cur.execute(
                    """
                    SELECT id_horario
                    FROM thorario_atencion
                ...
                if cur.fetchone():
                    return jsonify({
                        "ok": False,
                        "message": (
                            "Ya tienes un horario configurado. "
                            "Usa la opción de actualizar."
                        )
                    }), 409
                ...
                # Crear días del horario
                for dia in payload["dias"]:
                    cur.execute(
                        """
                        INSERT INTO thorario_dia
                            (id_horario, dia_semana)
                        VALUES
                            (%s, %s)
                        """,
                        (
        ...
        return jsonify({
            "ok": False,
            "message": f"No fue posible guardar el horario: {exc}"
        }), 500
...
def update_schedule():
    ...
    try:
            ...
            with conn.cursor() as cur:
                ...
                if not horario:
                    return jsonify({
                        "ok": False,
                        "message": (
                            "Aún no tienes un horario configurado. "
                            "Créalo primero."
                        )
                    }), 404

                # Validar sesiones activas fuera del nuevo horario
                cur.execute(
                    """
                    SELECT
                        s.id_sesion,
                        ...
                        s.hora_final
                    FROM tsesion s
                    JOIN testado_sesion es
                        ON es.id_estado_sesion = s.id_estado_sesion
                    WHERE s.id_tutor = %s
                      AND LOWER(es.txt_desc) = ANY(%s)
                      AND (
                            NOT (
                                EXTRACT(
                                    ISODOW FROM s.fec_sesion
                                )::int = ANY(%s)
                            )
                            OR s.hora_inicio < %s
                            OR s.hora_final > %s
                ...
                if conflictos:
                    detalle = [
                        (
                            f"{c['fec_sesion']} "
                            f"{str(c['hora_inicio'])[:5]}-"
                            f"{str(c['hora_final'])[:5]}"
                        )
                        for c in conflictos
                    ]
                    ...
                    return jsonify({
                        "ok": False,
                        "message": (
                            "No se puede actualizar: tienes sesiones "
                            "activas fuera del nuevo horario. "
                            "Reprograma o cancela estas sesiones primero: "
                            + "; ".join(detalle)
                        )
                    }), 409
        ...
        return jsonify({
            "ok": False,
            "message": f"No fue posible actualizar el horario: {exc}"
        }), 500
